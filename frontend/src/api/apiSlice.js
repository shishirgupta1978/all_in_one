...
const initialState = {	user: user ? jwt_decode(user.access)?.user : null, status: null,	result: null,properties:[]};
...
export const apiSlice = createSlice({
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(logout.fulfilled, (state) => {state.user.refresh=null;state.user = null;localStorage.removeItem("authTokens");toast.success("Successfully logout." )})
			.addCase(updateaccess.pending, (state) => {state.status = "pending";state.result = null;})
			.addCase(updateaccess.fulfilled, (state, action) => {state.status = "success";	state.result=action.payload; let a=JSON.parse(localStorage.getItem("authTokens"));a.access=action.payload?.access; localStorage.setItem("authTokens",JSON.stringify(a));})
			...
			.addCase(login.fulfilled, (state, action) => {state.status = "success";	state.result=action.payload;state.user = jwt_decode(action.payload?.access)?.user;localStorage.setItem("authTokens",JSON.stringify(action.payload));})
